...
async def register_user(
  user: _schemas.UserRequest, db: _orm.Session =_fastapi.Depends(_services.get_db)
    ...
    #call to check user with email exists
...
@app.get('/api/v1/user/current-user/', response_model=_schemas.UserResponse)
...
async def get_post_by_user(
    ...
    return await _services.get_posts_by_all(user = user, db=db)
...
async def update_post(
        post_id: int, post_requests = _schemas.PostRequest, db: _orm.Session = _fastapi.Depends(_services.get_db)
):
    db_post = await _services.get_post_detail(post_id = post_id, db = db)

    return await _services.update_post(post_requests = post_requests, post =db_post, db = db)